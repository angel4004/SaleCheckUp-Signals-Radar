...
## Цель проекта
Построить исследовательский контур, который помогает находить новые гипотезы роста SaleCheckUp через системный поиск, явное снижение неопределенности и controlled market scanning с накоплением:
...
Каждый research run в Manus должен быть:
- decision-driven;
- coverage-aware;
- explicit about completeness, blind spots, stopping reason и next search lane.
...
После этого:
1. выполнять research runs в Manus по актуальному approved contour из Git и фиксировать через stable approved paths, commit SHA и search-governance layer, который снижает неопределенность и делает coverage explicit;
