...
1. не различает `signal_type`, `resolution_status` и `evidence_strength`;
2. содержит конфликт между `run` как unit of account и `daily`-bound human-readable artifact naming.
...
3. В allowed values для `resolution_status`:
   - зафиксировать:
     - `accepted`
     - `rejected`
     - `hold`
...
5. В section, где описывается human-readable artifact:
   ...
   - зафиксировать, что обязательный human-readable artifact привязан к `run` или другому source-of-truth object run-level слоя;
   - `day` оставить только как navigation / filtering / grouping dimension, но не как mandatory artifact unit.

6. Если в контракте есть упоминание `reject` как semantic class:
   - убрать его из semantic classification layer;
   - отразить его смысл через `resolution_status = rejected`.

7. Если в контракте есть упоминание `hold` как класса:
   - не вводить его в `signal_type`;
   - отразить его только через `resolution_status = hold`.

## Explicit Non-Changes
- Не менять другие части output contract, не связанные с classification schema и artifact model.
- Не добавлять автоматически `hold_reason` и `next_action` в v0.2, если это не будет отдельно согласовано.
- Не переписывать весь документ целиком.

## Cross-Doc Sync Required
- `test_set_v0.1` -> должен быть синхронизирован с новой многослойной моделью.
...
## Risks
- Если обновить только `output_contract`, а `test_set` и `master_instruction` оставить старыми, появится новый cross-doc conflict.
