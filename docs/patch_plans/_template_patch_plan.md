# Patch Plan

## Status
Draft

## Related Decision
TBD

## Document Path
TBD

## Current Version
TBD

## Target Version
TBD

## Base Commit SHA
Optional

## Why This Patch Exists
Кратко опиши, какое противоречие, gap или sync-необходимость закрывает этот patch.

## Exact Changes
1. Что меняется в документе или наборе документов.
2. Какие stable approved paths или versioned handoff artifacts затрагиваются.
3. Какие control-layer обновления обязательны: `version_registry`, `change_log`, lifecycle/status, refs.

## Explicit Non-Changes
Явно перечисли, что этот patch не меняет.

## Cross-Doc Sync Required
Перечисли документы и индексы, которые нужно синхронизировать, если change scope их логически затрагивает.

## Risks
Зафиксируй риски broken refs, stale contour, lifecycle gaps или competing wording.

## Approval
Pending
