...
Текущий 	est_set_v0.1 больше не соответствует целевой архитектуре, потому что:
1. использует старую однослойную модель ожидаемого результата;
2. допускает hold как фактически отдельный класс, хотя в новой модели hold должен жить в esolution_status, а не в signal_type;
...
3. В cases, где раньше использовался eject как класс:
   - перевести это в:
...
- Если 
one не будет явно зафиксирован как допустимое значение signal_type, часть старых reject-cases останется семантически неоднозначной.
