...
2. старая логика, где `reject` трактуется как semantic class;
3. отсутствие явного различения между `signal_type`, `resolution_status` и `evidence_strength`;
...
2. Проверить, фигурирует ли `reject` как semantic signal class.
...
4. Проверить, содержит ли инструкция day-bound naming или day-bound artifact logic как обязательную primary model.
   - Если да, заменить это на run-centric artifact logic:
     - `run` = unit of execution
     - `run` = unit of account
     - `run` = source of truth artifact unit
...
- Не добавлять новые workflow fields типа `hold_reason` и `next_action` без отдельного решения.
