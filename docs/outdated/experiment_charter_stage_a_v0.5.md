# Experiment Charter — Stage A v0.5
...
- `VS Code/Codex` является repo/spec execution layer для изменения versioned документов и repo artifacts.
...
После этого:
1. выполнить следующий research run в Manus по актуальному набору approved docs;
2. если нужен repo/spec change, сначала зафиксировать versioned handoff artifact, а затем исполнять задачу в VS Code/Codex;
3. обновлять спецификацию только по итогам явного review loop и versioned sync.
